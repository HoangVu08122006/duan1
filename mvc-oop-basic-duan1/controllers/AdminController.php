...
function nhatKyAdd() {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        header("Location: index.php?act=nhatKy");
        exit;
    }

    ob_start();
    require './views/admin/nhatky/add.php';
    $content = ob_get_clean();
    require './views/layout_admin.php';
}


function nhatKyDetail() {
    $id = $_GET['id'] ?? 0;
    $model = new NhatKyModel();
    $nhatky = $model->getOne($id);
    ...
    ob_start();
    require './views/admin/nhatky/detail.php';
    $content = ob_get_clean();
    require './views/layout_admin.php';
}
...
function vanHanh() {
    require_once './models/BaoCaoVanHanhModel.php';
    $model = new BaoCaoVanHanhModel();
    $list = $model->getAll();
